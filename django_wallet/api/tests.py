        ...
        "results": [
            {
                ...
                "transactions": [{"id": 1, "txid": ANY, "wallet": 1, "amount": "0.000000000000000000"}],
            }
        ],
    }
...
@pytest.mark.parametrize(
    "balance, label, response_code", [(0, "test", 201), (0, "", 400), (2 * 64, "test", 201), (-1, "test", 400)]
)
@pytest.mark.django_db
...
def test_transaction_list(client, wallet, transaction):
    ...
    assert response.json() == {
        ...
        "next": None,
        "previous": None,
        "results": [{"id": ANY, "txid": ANY, "wallet": ANY, "amount": "0.000000000000000000"}],
    }


@pytest.mark.parametrize("amount, response_code", [(0, 201), (2 * 64, 201), (-1, 400)])
@pytest.mark.django_db
